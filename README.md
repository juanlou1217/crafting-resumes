...
对公司经历，它不会从“写了什么代码”直接跳到“有什么价值”。Skill 会把用户提供的代码、反馈、事故、指标、协作和交付物视为线索，主动推演多条完整候选业务逻辑，再与用户核实公司为什么投入、为什么现在优先、影响谁和多大、价值如何发生，以及具体问题怎样被判断和解决。
...
- 只知道局部实现，不知道公司为什么愿意投入资源、为什么先做这件事；
- 能列出范围、能力或客户语境，却无法说明真实采用、价值机制和分层结果；
...
- **完整业务逻辑推演**：根据局部线索生成 2–4 条从组织触发贯通到验证证据的候选逻辑，说明推理依据和可证伪条件；用户确认前始终保持为推测。
- **公司级经历深挖**：先恢复组织投入、优先级、影响规模和价值机制，再用“问题—判断—动作/调整—结果/归因/复盘”四层地图挖掘问题解决深度。
...
- **锚点经历优先**：将一至三段公司逻辑连贯、岗位相关且能承受追问的经历放在简历与面试准备前部，不平均展示任务流水账。
...
提供代码、任务、反馈、指标或一段自然叙述即可。Skill 先基于线索形成多条完整且待验证的业务逻辑，再用一个能区分真实解释的问题逐轮核实；确认后建立公司级业务案、四层经历地图和证据台账，不会把模型推测提前拼进简历。
...
| 输入与目标 | 可能输出 |
| --- | --- |
| 原始经历或零散材料 | 待验证候选业务逻辑、公司级业务案、四层经历地图、事实台账、当前唯一问题与风险 |
...
| `主导` | 在推动基础上，承担主要目标，并拥有关键决策权、推进节奏和结果责任 | `负责`、`推动`、`参与` |
...
可以。数字不是价值的唯一证据。公司为何投入、为什么优先、影响对象、价值机制、明确交付、约束、质量护栏、采用状态和决策取舍都可能形成有说服力的经历。Skill 会主动推演完整候选逻辑帮助回忆，但没有可靠口径时不补数字，也不会把规模、能力或采用直接改名成收入与增长。
