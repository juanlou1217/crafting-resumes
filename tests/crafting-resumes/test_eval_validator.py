...
VALIDATOR = ROOT / "tests/crafting-resumes/behavior/validate_eval_assets.py"
FORBIDDEN_TOKENS = (
    "private-profile-dir",
    "private-resume-dir",
...
class EvalValidatorTests(unittest.TestCase):
    ...
    def copy_eval_tree(self, destination: Path, source: Path = ROOT) -> None:
        shutil.copytree(
            source / "tests/crafting-resumes/behavior/cases",
            destination / "tests/crafting-resumes/behavior/cases",
        )
    ...
    def write_phase_manifests(
        ...
        cases_dir = root / "tests/crafting-resumes/behavior/cases"
        selected_cases = case_paths or sorted(cases_dir.glob("*.json"))
        manifest_dir = root / f"tests/crafting-resumes/manifests/{phase}"
        output_dir = root / f"tests/crafting-resumes/behavior/{phase}"
    ...
    def test_rejects_filename_id_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            case_path = (
                root
                / "tests/crafting-resumes/behavior/cases/01-course-project.json"
            )
    ...
    def test_rejects_raw_output_parent_symlink_outside_root(self) -> None:
        with (
            ...
            self.write_phase_manifests(root, "baseline")
            raw_directory = (
                root / "tests/crafting-resumes/behavior/baseline"
            )
    ...
    def test_rejects_cases_directory_symlink_loop_with_stable_error(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.copy_eval_tree(root)
            cases_directory = (
                root / "tests/crafting-resumes/behavior/cases"
            )
    ...
    def test_rejects_incomplete_case_set(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            (
                root
                / "tests/crafting-resumes/behavior/cases/12-complete-delivery.json"
            ).unlink()
    ...
    def test_rejects_parent_traversal_raw_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            manifest["raw_output_path"] = (
                "tests/crafting-resumes/behavior/baseline/../baseline/"
